...
def populate_new_teams_data(season, run_date):
    ...
    run_date = (
        datetime.today()
        if run_date == "today"
        else datetime.strptime(run_date, "%Y-%m-%d")
    )
    ...
    for team_id in teams_id:
        ...
        if data_retrieved.empty:
            try:
                logger.info(
                    f"Data not found for team {team_id} and run date {run_date}"
                )
                team_data[run_date] = utils.get_team_info(
                    headers=headers,
                    team_id=team_id,
                    season=season,
                    query_date=run_date.strftime("%Y-%m-%d"),
                )
